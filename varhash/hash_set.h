...
rehash ()
{
  ...
  unsigned newSize = primes[hSize_index+1];
  vector<HashEntry<T>*> tempV(newSize, nullptr);   // new, larger vector
  
  for (unsigned i = 0; i < hSize; ++i)
    { // For each occupied slot in current table...
      if (table[i] != nullptr
          && table[i]->info == Occupied)
        {
          HashEntry<T>* entry = table[i];
          
          //... find where in new table to place that entry
          unsigned h0 = entry->hash;
          unsigned h = h0 % (newSize);
          unsigned count = 0;
          while (tempV[h] != 0)
            {
              ++count;
              h = (h0 + count*count) % newSize;
            }
          tempV[h] = entry;
        }
    }
  
  // Change the table size and switch to the new vector
  ++hSize_index;
  hSize = newSize;
  table.swap (tempV);
}
...
find (const T& element, unsigned h0, bool stopAtDelete) const
{
  ...
  while (table[h] != nullptr &&
		  ((!stopAtDelete) || (table[h]->info != Deleted)) &&
		  ((table[h]->info != Occupied) || (!compare(table[h]->data, element))) &&
		  count < hSize)
    {
      ++count;
